# thumbnail_maker_using_MultiProcessing.py
...
class ThumbnailMakerService(object):
    ...
    def download_image(self, dl_queue, dl_size_lock):
        while not dl_queue.empty():
            try:
                ...
                self.img_queue.put(img_filename)       ##x_ref

                dl_queue.task_done()			##x_ref
            except queue.Empty:
    ...
    def make_thumbnails(self, img_url_list):
        ...
        dl_queue = Queue()    #dl_queue is not a picklable object.So, it can't be a member variable of class where multiprocessing is used.
        dl_size_lock = Lock() #same with threading.Lock().so smartly these are passed as aruments while created as local variables
        ...
        for _ in range(num_processes):
            p = multiprocessing.Process(target=self.perform_resizing)  #expects picklable object
            p.start()

        dl_queue.join() # block the queue,until all the downloads are complete, till the last x_ref marked above,after that this join completes and we can put poison pill
